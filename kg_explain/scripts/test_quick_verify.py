...
2. Re-run DTPD ranking (verify Fix 2: deduplication, Fix 3: non-drug filter)
3. Re-run ranker (verify Fix 5: safety gradient, phenotype boost)
...
# ====== Step 2: 重跑 DTPD + ranker 排序 ======
logger.info("=" * 60)
logger.info("Step 2: 重跑排序 (验证 Fix 2/3/5)")
logger.info("=" * 60)
...
# DTPD
dtpd_df = read_csv(cfg.output_dir / "dtpd_rank.csv")
logger.info("DTPD 排序: %d 行, %d 个药物", len(dtpd_df), dtpd_df["drug_normalized"].nunique())

# 检查非药物条目
non_drugs = ["cells", "placebo", "saline", "sodium chloride"]
for term in non_drugs:
    matches = dtpd_df[dtpd_df["drug_normalized"].str.contains(term, case=False, na=False)]
    if not matches.empty:
        logger.warning("⚠ DTPD 仍含非药物 '%s': %d 行", term, len(matches))
    else:
        logger.info("✓ DTPD 无 '%s' 条目", term)

# 检查路径重复
dtpd_paths = pd.read_json(cfg.output_dir / "dtpd_paths.jsonl", lines=True)
logger.info("DTPD evidence paths: %d 行", len(dtpd_paths))

# 找一个药物看是否有重复路径
if not dtpd_paths.empty:
    sample_drug = dtpd_paths["drug"].value_counts().index[0]
    sample_paths = dtpd_paths[dtpd_paths["drug"] == sample_drug]
...
# Ranker
rank_df = read_csv(cfg.output_dir / "drug_disease_rank.csv")
logger.info("\n排序: %d 行, %d 个药物", len(rank_df), rank_df["drug_normalized"].nunique())

# 检查 safety_penalty 梯度
sp = pd.to_numeric(rank_df["safety_penalty"], errors="coerce")
...
# ====== 打印前 100 行 ======
logger.info("=" * 60)
logger.info("前 100 行排序结果 (按 final_score 降序)")
logger.info("=" * 60)

rank_sorted = rank_df.sort_values("final_score", ascending=False).head(100)
pd.set_option("display.max_columns", None)
pd.set_option("display.width", 200)
pd.set_option("display.max_colwidth", 30)
print(rank_sorted.to_string(index=False))

logger.info("\n完成! 共 %d 行排序输出", len(rank_df))
