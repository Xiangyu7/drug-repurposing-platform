...
launch_pipeline() {
    ...
    nohup env \
        ...
        KG_PY="${runtime_kg}" \
        LLM_PY="${runtime_llm}" \
        ${STEP_TIMEOUT:+STEP_TIMEOUT="${STEP_TIMEOUT}"} \
        ${TIMEOUT_CROSS_KG_SIGNATURE:+TIMEOUT_CROSS_KG_SIGNATURE="${TIMEOUT_CROSS_KG_SIGNATURE}"} \
        ${TIMEOUT_ORIGIN_KG_CTGOV:+TIMEOUT_ORIGIN_KG_CTGOV="${TIMEOUT_ORIGIN_KG_CTGOV}"} \
        ${TIMEOUT_CROSS_DSMETA:+TIMEOUT_CROSS_DSMETA="${TIMEOUT_CROSS_DSMETA}"} \
        ${TIMEOUT_CROSS_SIGREVERSE:+TIMEOUT_CROSS_SIGREVERSE="${TIMEOUT_CROSS_SIGREVERSE}"} \
        ${TIMEOUT_CROSS_ARCHS4:+TIMEOUT_CROSS_ARCHS4="${TIMEOUT_CROSS_ARCHS4}"} \
        ${TIMEOUT_LLM_STEP6:+TIMEOUT_LLM_STEP6="${TIMEOUT_LLM_STEP6}"} \
        ${TIMEOUT_LLM_STEP7:+TIMEOUT_LLM_STEP7="${TIMEOUT_LLM_STEP7}"} \
        ${TIMEOUT_LLM_STEP8:+TIMEOUT_LLM_STEP8="${TIMEOUT_LLM_STEP8}"} \
        ${TIMEOUT_LLM_STEP9:+TIMEOUT_LLM_STEP9="${TIMEOUT_LLM_STEP9}"} \
