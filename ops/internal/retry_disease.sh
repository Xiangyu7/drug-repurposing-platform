...
#
# Usage:
#   bash ops/internal/retry_disease.sh atherosclerosis                    # retry Direction B (default)
#   bash ops/internal/retry_disease.sh atherosclerosis --mode dual        # retry A + B
#   bash ops/internal/retry_disease.sh atherosclerosis --mode cross_only  # retry A only
#
# This script:
...
#   4. Runs the pipeline for that disease only
# ============================================================

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OPS_DIR="${ROOT_DIR}/ops"
RUNNER="${OPS_DIR}/internal/runner.sh"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: bash ops/internal/retry_disease.sh <disease_key> [options]"
            echo ""
            echo "Options:"
            ...
            echo ""
            echo "Examples:"
            echo "  bash ops/internal/retry_disease.sh atherosclerosis"
            echo "  bash ops/internal/retry_disease.sh heart_failure --mode dual"
            echo "  bash ops/internal/retry_disease.sh stroke --mode cross_only"
            exit 0
            ;;
        -*)
...
if [[ -z "${DISEASE_KEY}" ]]; then
    fail "Usage: bash ops/internal/retry_disease.sh <disease_key> [--mode dual|origin_only|cross_only]"
fi

# ── Look up disease metadata from existing lists ──
disease_line=""
for list_file in "${OPS_DIR}/internal/disease_list_day1_dual.txt" "${OPS_DIR}/internal/disease_list_day1_origin.txt" "${OPS_DIR}/internal/disease_list_b_only.txt" "${OPS_DIR}/disease_list.txt"; do
    if [[ -f "${list_file}" ]]; then
