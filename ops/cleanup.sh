...
#
# Usage:
#   bash ops/cleanup.sh                # show disk usage summary
#   bash ops/cleanup.sh --work 7       # remove work dirs older than 7 days
#   bash ops/cleanup.sh --quarantine 3 # remove quarantine dirs older than 3 days
#   bash ops/cleanup.sh --kg-cache     # remove kg_explain HTTP cache
#   bash ops/cleanup.sh --dsmeta-work  # remove all dsmeta work dirs
#   bash ops/cleanup.sh --logs 30      # remove logs older than 30 days
#   bash ops/cleanup.sh --all 7        # all of the above (7 day threshold)
#   bash ops/cleanup.sh --dry-run --all 7  # show what would be deleted
# ============================================================
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: bash ops/cleanup.sh [action] [options]"
            echo ""
            echo "Actions:"
...
if [[ -z "${ACTION}" ]]; then
    ...
    echo ""
    echo "Tip: bash ops/cleanup.sh --all 7      # clean everything > 7 days"
    echo "     bash ops/cleanup.sh --dry-run --all 7  # preview first"
    exit 0
fi
