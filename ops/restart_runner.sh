...
#
# Usage:
#   bash ops/restart_runner.sh                  # restart all active runners
#   bash ops/restart_runner.sh --runner dual    # restart specific runner
#   bash ops/restart_runner.sh --stop           # stop only, don't restart
#
# This script:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: bash ops/restart_runner.sh [options]"
            echo ""
            echo "Options:"
            echo "  --runner <name>  Target specific runner (dual, origin_only, cross_only, m1_serial)"
...
# ── Restart ──
echo ""
echo "To restart, use one of:"
echo "  bash ops/start_day1_aliyun.sh           # cloud: dual + origin parallel"
echo "  bash ops/start_m1_serial.sh             # mac: serial mode"
echo "  bash ops/quickstart.sh --run-only       # quickstart runner"
echo ""
echo "Or run a single disease:"
echo "  bash ops/quickstart.sh --single <disease> --mode dual"
echo ""
